...
def test_no_upgrade_unless_requested(script: KpipTestEnvironment) -> None:
    """No upgrade if not specifically requested."""
    script.kpip_install_local("simplewheel==1.0")
    result = script.kpip_install_local("simplewheel")
    assert not result.files_created, (
        "kpip install INITools upgraded when it should not have"
    )


def test_invalid_upgrade_strategy_causes_error(script: KpipTestEnvironment) -> None:
...
def test_only_if_needed_does_not_upgrade_deps_when_satisfied(
    ...
    script.kpip_install_local("simple==2.0")
    result = script.kpip_install_local(
        "--upgrade",
...
def test_upgrade_to_specific_version(script: KpipTestEnvironment) -> None:
    """It does upgrade to specific version requested."""
    script.kpip_install_local("simplewheel==1.0")
    result = script.kpip_install_local("simplewheel==2.0")
    assert result.files_created, "kpip install with specific version did not upgrade"
...
def test_upgrade_if_requested(script: KpipTestEnvironment) -> None:
    """And it does upgrade if requested."""
    script.kpip_install_local("simplewheel==1.0")
    result = script.kpip_install_local("--upgrade", "simplewheel")
    assert result.files_created, "kpip install --upgrade did not upgrade"
...
def test_upgrade_force_reinstall_newest(script: KpipTestEnvironment) -> None:
    ...
    result2 = script.kpip_install_local("--upgrade", "--force-reinstall", "simplewheel")
    assert result2.files_updated, "upgrade to simplewheel 2.0 failed"
    result3 = script.kpip("uninstall", "simplewheel", "-y")
    assert_all_changes(result, result3, [script.venv / "build", "cache"])


def test_uninstall_before_upgrade(script: KpipTestEnvironment) -> None:
    """Automatic uninstall-before-upgrade."""
    result = script.kpip_install_local("simplewheel==1.0")
    result.did_create(script.site_packages / "simplewheel")
    result2 = script.kpip_install_local("simplewheel==2.0")
...
@pytest.mark.network
def test_uninstall_before_upgrade_from_url(script: KpipTestEnvironment) -> None:
    """Automatic uninstall-before-upgrade from URL."""
    result = script.kpip("install", "INITools==0.2")
    ...
    result3 = script.kpip("uninstall", "initools", "-y")
    assert_all_changes(result, result3, [script.venv / "build", "cache"])


@pytest.mark.network
def test_upgrade_to_same_version_from_url(script: KpipTestEnvironment) -> None:
    ...
    """
    result = script.kpip("install", "INITools==0.3")
    result.did_create(script.site_packages / "initools")
    result2 = script.kpip(
        "install",
        "https://files.pythonhosted.org/packages/source/I/INITools/INITools-0.3.tar.gz",
...
def test_upgrade_from_reqs_file(script: KpipTestEnvironment) -> None:
    ...
    install_result = script.kpip_install_local("-r", req_file)
    ...
    script.kpip_install_local("--upgrade", "-r", req_file)
    uninstall_result = script.kpip("uninstall", "-r", req_file, "-y")
    assert_all_changes(
        install_result,
        uninstall_result,
...
def test_uninstall_rollback(script: KpipTestEnvironment, data: TestData) -> None:
...
def test_should_not_install_always_from_cache(
    ...
    """If there is an old cached package, kpip should download the newer version
    Related to issue #175
    """
    script.kpip_install_local("simplewheel==2.0")
    script.kpip("uninstall", "-y", "simplewheel")
    result = script.kpip_install_local("simplewheel==1.0")
...
def test_install_with_ignoreinstalled_requested(script: KpipTestEnvironment) -> None:
    """Test old conflicting package is completely ignored"""
    script.kpip_install_local("simplewheel==1.0")
    result = script.kpip_install_local("-I", "simplewheel==2.0")
    assert result.files_created, "kpip install -I did not install"
...
def test_upgrade_vcs_req_with_no_dists_found(
    ...
    script.kpip("install", req)
    result = script.kpip("install", "-U", req)
    assert not result.returncode


@pytest.mark.network
def test_upgrade_vcs_req_with_dist_found(script: KpipTestEnvironment) -> None:
    ...
    script.kpip("install", req, expect_stderr=True)
    result = script.kpip("install", "-U", req, expect_stderr=True)
...
def test_modifying_kpip_presents_error(script: KpipTestEnvironment) -> None:
    result = script.kpip(
        "install",
        "kpip",
        "--force-reinstall",
