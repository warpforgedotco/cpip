...
``CPIP_CACHE_DIR`` do not move it -- two installers targeting one
environment must meet at the same path whatever their cache settings say --
and ``cpip cache purge``, which reaps whole ``v*`` roots, leaves this
...
def lock_dir() -> str:
    ...
    return os.path.join(user_cache_dir("cpip"), "locks")
